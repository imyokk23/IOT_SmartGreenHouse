...
# --- SETUP SERVER ---
app = Flask(__name__)
# CORS diaktifkan agar script.js dari folder frontend & Mobile App bisa akses API ini
CORS(app, resources={r"/*": {"origins": "*"}})

# --- SETUP DATABASE (MySQL) ---
# Ganti user/password sesuai konfigurasi XAMPP kamu
user = 'root'
password = ''
host = 'localhost'
...
@app.route('/')
def home():
    return jsonify({"message": "Server Smart Home C2 Ready!", "tim": "Kelompok Badagh"})
...
def get_history():
    try:
        logs = SensorLog.query.order_by(SensorLog.timestamp.desc()).limit(10).all()
...
def control_device():
    try:
        ...
        if component and action:
            ...
            return jsonify({"status": "success", "message": f"Berhasil set {component} ke {action}"})
        return jsonify({"status": "error", "message": "Invalid data"}), 400
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500

# --- CALLBACK UNTUK MQTT HANDLER ---
def update_sensor_data(data_baru):
    global latest_data
    print(f"[DATA MASUK DARI ESP] {data_baru}")
    
    # 1. Update RAM (Mapping Data)
    ...
    try:
        ...
        print(f"[DB ERROR] Gagal simpan ke MySQL: {e}")
