...
// Cek lagi ipconfig jika pindah WiFi/Hotspot
...
function showPage(pageId, linkRef) {
    ...
    // Reset style semua link navigasi
    ...
    // Highlight menu yang aktif (jika diklik dari sidebar)
    if(linkRef) {
...
function updateUI(data) {
    ...
    // [UPDATE] Grafik Realtime (Opsional: Update data chart jika perlu realtime movement)
    // Untuk simplifikasi, chart diupdate via reload halaman atau logika tambahan chart.update()
}
...
function updatePumpUI(status) {
    ...
    buttons.forEach(btn => {
        const textSpan = btn.querySelector('.btn-text'); // Cari span teks di dalam tombol
        const icon = btn.querySelector('i'); // Cari ikon
...
async function toggleWatering() {
    ...
    try {
        ...
        console.log(`Mengirim perintah: ${actionToSend}`);
...
// Fungsi tambahan untuk Kamera (Refresh / Fallback)
...
document.addEventListener('DOMContentLoaded', function() { 
    ...
    // Inisialisasi Chart (Data Dummy Awal, nanti bisa diupdate via API history)
    const ctxSuhu = document.getElementById('chart-suhu');
    if(ctxSuhu) new Chart(ctxSuhu, { type: 'line', data: { labels: [1,2,3,4,5], datasets: [{ data: [25,26,25,27,26], borderColor: '#3b82f6', tension: 0.4 }] }, options: commonOptions });
    
    const ctxHum = document.getElementById('chart-kelembapan');
    if(ctxHum) new Chart(ctxHum, { type: 'line', data: { labels: [1,2,3,4,5], datasets: [{ data: [60,65,62,60,61], borderColor: '#14b8a6', tension: 0.4 }] }, options: commonOptions });
    
    const ctxCahaya = document.getElementById('chart-cahaya');
    if(ctxCahaya) new Chart(ctxCahaya, { type: 'line', data: { labels: [1,2,3,4,5], datasets: [{ data: [100,200,150,300,120], borderColor: '#eab308', tension: 0.4 }] }, options: commonOptions });
});